...
maintainer listed in [`MAINTAINERS.md`][maintainers] through the contact details
on that person's GitHub profile -- currently [@astrogilda][maintainer-profile].
A report that does not need to be private may instead be opened as an issue on
...
## Reporting conduct by a maintainer

`GOVERNANCE.md` does not let a maintainer review their own crosswalk, and the
same refusal applies to conduct: a maintainer does not handle a report about
themselves. This repository currently lists one maintainer, so there is no
...
[maintainer-profile]: https://github.com/astrogilda
